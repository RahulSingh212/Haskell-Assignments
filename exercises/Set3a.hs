...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a_val b_val = if measure b_val > measure a_val then b_val else a_val
...
check_num_func :: [Char] -> Bool
check_num_func idx = idx == idx
...
add_const_val = 2
firstHalf idx_val = take hc idx_val
    where
        hc = div leni add_const_val + mod leni add_const_val
        leni = length idx_val

palindrome idx_val = reverse idx_val == idx_val
...
capitalizeFirst [] = empty_list
capitalizeFirst (a: ac) = toUpper a : ac
...
whileRight check x_val = case check x_val of 
    Left output_val -> output_val
    Right new_x_val -> whileRight check new_x_val
...
bomb 0 = Left str_boom_val
bomb x = Right (x-const_one_val)
...
joinToLength len strings = [result 
            | str1 <- strings, 
            str2 <- strings, 
...
multiCompose [] x_val = x_val
multiCompose (f:fs) x_val = f (multiCompose fs x_val)
...
multiApp :: ([x_val] -> y_val) -> [z_val -> x_val] -> z_val -> y_val
multiApp f gs x_val = f $ applyAll gs x_val

applyAll :: [c_val -> a_val] -> c_val -> [a_val]
applyAll [] _ = empty_list
applyAll (f:fs) x_val = f x_val : applyAll fs x_val
...
interpreter commands = processCommands 0 0 commands empty_list

str_up_val = "up"
str_down_val = "down"
str_left_val = "left"
str_right_val = "right"
str_printX_val = "printX"
str_printY_val = "printY"
const_one_val = 1

processCommands :: Int -> Int -> [String] -> [String] -> [String]
processCommands _ _ [] result = result
processCommands x y (cmd:cmds) result
    | cmd == str_up_val     = processCommands x (y + const_one_val) cmds result
    | cmd == str_down_val   = processCommands x (y - const_one_val) cmds result
    | cmd == str_left_val   = processCommands (x - const_one_val) y cmds result
    | cmd == str_right_val  = processCommands (x + const_one_val) y cmds result
    | cmd == str_printX_val = processCommands x y cmds (result ++ [show x])
    | cmd ==str_printY_val = processCommands x y cmds (result ++ [show y])
    | otherwise       = processCommands x y cmds result
